...
(define (apply-procedure f store x) (f store x))
...
(define (to-proc-exp var vars body)
  (cond
    [(null? vars) (proc-exp var body)]
    [else
     (proc-exp var (to-proc-exp (car vars) (cdr vars) body))]))
...
(define (value-of exp env store)
  (cases expression exp
    ...
    (proc*-exp (var vars body)
      (value-of (to-proc-exp var vars body) env store))
      ...
      (cases answer (value-of e1 env store)
        (an-answer (val1 new-store1)
          (cases answer (value-of e2 env new-store1)
            (an-answer (val2 new-store2)
              (apply-procedure (expval->proc val1) store val2))))))
