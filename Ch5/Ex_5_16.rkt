...
; StmtCont = ExpVal -> ExpVal

; apply-stmt-cont :  StmtCont -> ()
(define (apply-stmt-cont cont) (cont))

; end-stmt-cont : StmtCont
(define (end-stmt-cont)
  (lambda () '()))

; asgn-stmt-cont : Var * Exp * Env * StmtCont -> StmtCont
(define (asgn-stmt-cont var exp env cont)
  (lambda ()
     (begin
...
; print-stmt-cont : Exp * Env * StmtCont -> StmtCont
(define (print-stmt-cont exp env cont)
  (lambda () 
    (write (expval->any (value-of exp env)))
    (display "\n")
    (apply-stmt-cont cont)))

; read-stmt-cont : Var * Env * StmtCont -> StmtCont
(define (read-stmt-cont var env cont)
  (lambda ()
    (setref! (apply-env env var) (num-val (read)))
    (apply-stmt-cont cont)))

; seq-stmt-cont : List Stmt * Env * StmtCont -> StmtCont
(define (seq-stmt-cont stmts env cont)
  (lambda ()
    (cond
...
; if-stmt-cont : Exp * Stmt * Stmt * Env * StmtCont -> StmtCont
(define (if-stmt-cont e s1 s2 env cont)
  (lambda ()
    (if (expval->bool (value-of e env))
        (exec-stmt s1 env cont)
        (exec-stmt s2 env cont))))

; while-stmt-cont : Exp * Stmt * Env * StmtCont -> StmtCont
(define (while-stmt-cont e s env cont)
  (lambda ()
    (if (expval->bool (value-of e env))
