...
;     | extend-env-rec Ident Ident Exp Env
...
(define (car-cont cont)
  (lambda (val)
    (apply-cont cont (any->expval (car (expval->list val))))))
...
(define (head-cont tail-exp env cont)
  (lambda (head)
    (value-of/k tail-exp env (tail-cont (expval->any head) cont))))
...
(define (value-of/k exp env cont)
  (cases expression exp
    ...
    (null?-exp (e)
      (value-of/k e env (null?-cont cont)))

    (car-exp (e)
      (value-of/k e env (car-cont cont)))

    (cdr-exp (e)
      (value-of/k e env (cdr-cont cont)))
