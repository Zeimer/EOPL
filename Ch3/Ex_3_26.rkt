...
(define (apply-env k e)
  (cases env e
    ...
    (extend-env (k2 v e2) (if (eqv? k k2) v (apply-env k e2)))))

(define (has-binding? k e)
  (cases env e
    (empty-env () #f)
    (extend-env (k2 v e2) (if (eqv? k k2) #t (has-binding? k e2)))))

(define (filter-env p e)
  (cases env e
    (empty-env () (empty-env))
    (extend-env (k v e2)
                (if (p k)
                    (extend-env k v (filter-env p e2))
                    (filter-env p e2)))))
...
(define (apply-procedure f x)
  (cases proc f
    (procedure (var body env)
               (value-of body (extend-env var x env)))))
